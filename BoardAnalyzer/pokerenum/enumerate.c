...
static enum_gameparams_t enum_gameparams[] = {
  ...
  { game_holdem_nosuits,    2,  2,  5, 0, 1, "Holdem Hi No Suits" },
};
...
// ACP - hardwired for 2 played HE
  ...
  } else if (game == game_holdem_nosuits) {
      StdDeck_CardMask sharedCards;
      if (nboard == 0) {
    	return 0;
        //DECK_ENUMERATE_5_CARDS_D(StdDeck, sharedCards, dead, INNER_LOOP_HOLDEM);
      } else if (nboard == 3) {
        DECK_ENUMERATE_2_CARDS_NOSUITS_D(StdDeck, sharedCards, dead, INNER_LOOP_HOLDEM);
      } else if (nboard == 4) {
      	return 0;
        //DECK_ENUMERATE_1_CARDS_D(StdDeck, sharedCards, dead, INNER_LOOP_HOLDEM);
      } else if (nboard == 5) {
